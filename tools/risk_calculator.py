...
def calculate_risk_metrics(
    ...
    try:
        ...
        tr = np.maximum(
            highs - lows,
            np.maximum(
                np.abs(highs - np.roll(closes_14, 1)),
                np.abs(lows - np.roll(closes_14, 1))
            )
        )[1:]  # İlk değeri atla (roll artifact)
        ...
        result = {
            ...
            "volatility": {
                ...
                "annualized": f"{annualized_volatility*100:.1f}%",
                "regime": vol_regime,
                "atr_14": round(atr, 2),
            },
            "risk_metrics": {
                "var_daily": f"{daily_var*100:.2f}%",
                "var_daily_usd": round(daily_var * current_price, 2),
                "var_annual": f"{annual_var*100:.1f}%",
                "max_drawdown_6mo": f"{max_drawdown*100:.1f}%",
            ...
            "position_sizing": {
                ...
                "position_pct_of_portfolio": f"{position_pct:.1f}%",
            },
            "trade_levels": {
                "entry_price": round(current_price, 2),
                "stop_loss": stop_loss_price,
                "stop_loss_pct": f"{(stop_loss_distance/current_price)*100:.1f}%",
                "take_profit": take_profit_price,
                "take_profit_pct": f"{(take_profit_distance/current_price)*100:.1f}%",
        ...
        if vol_regime == "EXTREME":
            recs.append("⚠️ EXTREME volatility - consider reducing position size by 50%")
        if max_drawdown < -0.20:
            recs.append("⚠️ Significant drawdown in last 6 months - exercise caution")
        if rr_ratio < 1.5:
            recs.append("⚠️ Risk/Reward ratio below 1.5 - consider wider take-profit")
        if position_pct > 20:
            recs.append("⚠️ Position exceeds 20% of portfolio - consider reducing")
        if sharpe_approx > 1:
            recs.append("✅ Good risk-adjusted returns historically (Sharpe > 1)")
        if vol_regime == "LOW":
            recs.append("✅ Low volatility environment - favorable for entry")
