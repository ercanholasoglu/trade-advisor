...
def get_fundamental_data(ticker: str) -> str:
    ...
    try:
        ...
        # Market cap okunabilir format
        mc = profile["market_cap"]
        if isinstance(mc, (int, float)):
            if mc >= 1e12:
                profile["market_cap_formatted"] = f"${mc/1e12:.2f}T"
            elif mc >= 1e9:
                profile["market_cap_formatted"] = f"${mc/1e9:.2f}B"
            elif mc >= 1e6:
                profile["market_cap_formatted"] = f"${mc/1e6:.2f}M"
        ...
        for k in ["revenue", "free_cash_flow"]:
            ...
            if isinstance(v, (int, float)):
                if abs(v) >= 1e9:
                    financials[f"{k}_formatted"] = f"${v/1e9:.2f}B"
                elif abs(v) >= 1e6:
                    financials[f"{k}_formatted"] = f"${v/1e6:.2f}M"

        for k in ["revenue_growth", "gross_margin", "operating_margin", "profit_margin",
                   "roe", "roa", "earnings_growth"]:
            v = financials[k]
            if isinstance(v, (int, float)):
                financials[k] = f"{v*100:.1f}%"
        ...
        if isinstance(pe, (int, float)):
            if pe < 15:
                signals.append("LOW_PE_UNDERVALUED")
            elif pe > 30:
                signals.append("HIGH_PE_OVERVALUED")

        peg = valuation["peg_ratio"]
        if isinstance(peg, (int, float)):
            if peg < 1:
                signals.append("PEG_UNDERVALUED")
            elif peg > 2:
                signals.append("PEG_OVERVALUED")

        rec = target["recommendation"]
        if rec in ["buy", "strongBuy", "strong_buy"]:
            signals.append("ANALYSTS_BULLISH")
        elif rec in ["sell", "strongSell", "strong_sell"]:
            signals.append("ANALYSTS_BEARISH")
