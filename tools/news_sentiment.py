...
from tools.cache import cache, TTL_NEWS


@cache(ttl=TTL_NEWS)
def _fetch_news_sentiment(ticker: str, company_name: str) -> str:
    """Cached inner: fetches news + runs sentiment."""
    import json
    import os
    ...
    try:
        ...
        if not results:
            return json.dumps({"ticker": ticker.upper(), "error": "No recent news found", "news_count": 0})
        ...
        if hf_token:
            try:
                ...
                test_resp = requests.post(
                    ...
                    json={"inputs": results[0].get("title", "test")}, timeout=10)
        ...
        for r in results:
            ...
            if finbert_available:
                sentiment, score = _finbert_sentiment(title[:512], hf_token)
            else:
                sentiment, score = _keyword_sentiment(title + " " + body)
            sentiment_scores.append(score)
            articles.append({"title": title, "source": r.get("source", "Unknown"),
                "date": r.get("date", "Unknown"), "url": r.get("url", ""),
        ...
        if avg_score > 0.2: overall_sentiment = "BULLISH"
        elif avg_score < -0.2: overall_sentiment = "BEARISH"
        else: overall_sentiment = "NEUTRAL"

        return json.dumps({"ticker": ticker.upper(), "news_count": len(articles),
            "sentiment_method": "FinBERT" if finbert_available else "keyword",
            "overall_sentiment": overall_sentiment, "avg_sentiment_score": round(avg_score, 3),
            "sentiment_breakdown": {"positive": pos_articles, "negative": neg_articles,
                "neutral": sum(1 for s in sentiment_scores if s == 0)},
            "articles": articles}, indent=2)
...
def get_news_sentiment(ticker: str, company_name: str = "") -> str:
    ...
    """
    return _fetch_news_sentiment(ticker, company_name)
