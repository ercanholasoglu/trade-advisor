...
def get_market_overview() -> str:
    ...
    Use this before making any trade recommendation to understand the macro environment.
    ...
    try:
        ...
        # VIX
        vix_price, vix_change, vix_change_pct = get_change(vix_ticker)
        if vix_price is not None:
            if vix_price < 15:
                vix_regime = "LOW_FEAR (Complacency)"
            elif vix_price < 20:
                vix_regime = "NORMAL"
            elif vix_price < 30:
                vix_regime = "ELEVATED_FEAR"
            else:
                vix_regime = "HIGH_FEAR (Panic)"
        else:
            vix_regime = "N/A"
        ...
        sorted_sectors = dict(
            sorted(
                sector_data.items(),
                key=lambda x: float(x[1]["change_pct"].replace("%", "").replace("+", "")),
                reverse=True,
            )
        )

        # Genel piyasa rejimi değerlendirmesi
        positive_indices = sum(
            1 for d in index_data.values() if d.get("change_pct", "").startswith("+") or d.get("change_pct", "") == "+0.00%"
        )
        ...
        result = {
            ...
            "vix": {
                "value": vix_price,
                "change_pct": f"{vix_change_pct:+.2f}%" if vix_change_pct else "N/A",
                "regime": vix_regime,
            },
            "indices": index_data,
            "key_assets": asset_data,
            "sector_performance": sorted_sectors,
            "top_sectors": list(sorted_sectors.keys())[:3],
            "bottom_sectors": list(sorted_sectors.keys())[-3:],
        }
